...
use crate::widget::overlay::cell_overlay::{GROUP_ICON_SIZE, TOGGLE_ICON_SIZE};
...
impl<B> cell2::Renderer for Renderer<B>
where
    B: Backend + backend::Text,
{
    ...
    fn draw<Message>(
        ...
        let (text_label, new_mouse_interaction) = item.draw(
            self,
            &Defaults {
                text: defaults::Text {
...
impl<B> crate::widget::overlay::cell_overlay::Renderer for Renderer<B>
where
    B: Backend + backend::Text,
{
    type Style = Box<dyn StyleSheet>;
    ...
    fn draw<Message>(
        ...
        env: DrawEnvironment<'_, Self::Defaults, Self::Style, ()>,
        state: &State,
        section: &Vec<Entry<'_, Message, Self>>,
    ) -> Self::Output {
        let children = env.layout.children();
