...
impl Opts {
    async fn load(opt: Opts) -> Result<(), std::io::Error> {
        let Opts { config, wdbpath, vcdpath } = opt;
        Ok(())
    }
}
...
fn main() {
    ...
    let mut settings: Settings<Opts> = Settings { flags: opts, ..Settings::default() };
    env_logger::init();
    Wave2::run(settings);
}
    ...
    fn view(&mut self) -> Element<Message> {
        match self {
            Content::HierNav(hier_mod) => {
                hier_mod.view().map(move |message| Message::HNMessage(message))
            }
            Content::SigView(sig_view) => {
                sig_view.view().map(move |message| Message::SVMessage(message))
            }
            Content::ModNav(module_nav) => {
                module_nav.view().map(move |message| Message::MNMessage(message))
            }
        }
    }
}
...
impl Application for Wave2 {
    ...
    fn new(flags: Opts) -> (Wave2, Command<Self::Message>) {
        (Wave2::Loading, Command::perform(Opts::load(flags), Message::Loaded))
    }
        ...
        match self {
            Wave2::Loading => {
                match message {
                    Message::Loaded(Ok(void)) => {
                        ...
                        let (mn_pane, _) =
                            panes.split(pane_grid::Axis::Vertical, &sv_pane, mod_nav).unwrap();
                        panes.swap(&mn_pane, &sv_pane);
                        let (hn_pane, _) =
                            panes.split(pane_grid::Axis::Horizontal, &mn_pane, hier_nav).unwrap();
            ...
            Wave2::Loaded(state) => {
                match message {
                    ...
                    Message::HNMessage(_) => {
                        state.panes.get_mut(&state.hn_pane).unwrap().update(message)
                    }
                    Message::MNMessage(_) => {
                        state.panes.get_mut(&state.mn_pane).unwrap().update(message)
                    }
    ...
    fn view(&mut self) -> Element<Self::Message> {
        match self {
            ...
            Wave2::Loaded(State { panes, menu_bar, .. }) => {
                //all_content.into()
...
fn loading_message() -> Element<'static, Message> {
    Container::new(
        Text::new("Loading...").horizontal_alignment(HorizontalAlignment::Center).size(50),
    )
    .width(Length::Fill)
    .height(Length::Fill)
    .center_y()
