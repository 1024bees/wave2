...
impl PuddleBuilder {
    ...
    pub fn add_signal(&mut self, command: Command, timestamp: Toffset) -> Result<(), Waverr> {
        let time_delta = u16::try_from(timestamp - self.base)
            .expect("Puddles are much too large; probably an error with how this is called")
            & 0xfff;
...
pub mod tests {
    ...
    fn build_rand_puddles<Width: Distribution<u64>, SignalFreq: Distribution<bool>>(
        base: Toffset,
        num_ids: u32,
        signal_widths: Width,
        freq: SignalFreq,
                ...
                if freq.sample(&mut rand::thread_rng()) {
                    pb.add_signal(
                        Command::ChangeVector(
                            (i as u64).into(),
                            num_to_vec(value_dist.sample(&mut rand::thread_rng()), width as u8),
                        ),
                        time + base,
                    )
                    .unwrap();
